...
function releaseES(){
    ...
    cd  ${desfilepath}
    chomd -R 775 ${desfilepath}/${foldername}
}
function updateES(){
   nodename=$1
...
function updatefilebeat(){
    currentHostname=$1
    currentIP=$2
   fb_path='/usr/local/filebeat.yml'
...
function startfilebeat(){
    cd /usr/local/
    ./filebeat-1.0.0-x86_64/filebeat -e -c ./filebeat.yml   &
}
...
function updatelogstash(){
    ipaddr=$1
   ls_path='/usr/local/beatsfinal.conf'
...
function startlogstash(){ 
   cd    /usr/local
   ./logstash-1.5.6/bin/logstash -f ./beatsfinal.conf &
}
